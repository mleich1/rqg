...
thread1_init:
    INSTALL SONAME 'file_key_management' { $a_refresh = time(); $b_refresh = time() + 15; $c_refresh = time() + 30 ; $m1 = '/*'; $m2 = '*/' ; return undef };

thread1:
    {if ($a_refresh < time()) { $a_refresh = time() + 60; $m1 = ''; $m2 = '' ; return undef } else { $m1 = '/*'; $m2 = '*/' } ; return undef } $m1 SET GLOBAL innodb_encryption_threads = encryption_threads $m2 ; query | 
    {if ($b_refresh < time()) { $b_refresh = time() + 60; $m1 = ''; $m2 = '' ; return undef } else { $m1 = '/*'; $m2 = '*/' } ; return undef } $m1 SET GLOBAL innodb_encryption_rotate_key_age = encryption_rotate_key_age $m2 ; query | 
    {if ($c_refresh < time()) { $c_refresh = time() + 60; $m1 = ''; $m2 = '' ; return undef } else { $m1 = '/*'; $m2 = '*/' } ; return undef } $m1 set_compression_algorithm $m2 ; query ;
