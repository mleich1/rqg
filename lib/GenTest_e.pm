...
sub tmpdir {
    if (defined $tmpdir) {
        ...
        Carp::cluck("ERROR: The tmpdir was not set.");
...
sub set_tmpdir {
    ...
    $tmpdir = $new_tmpdir;
    if (not -e $new_tmpdir) {
        Carp::cluck("ERROR: The assigned tmpdir ->$new_tmpdir<- does not exist.");
        safe_exit(STATUS_INTERNAL_ERROR);
    } else {
...
sub isoTimestamp {
    ...
    my ($sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst) =
        defined $datetime ? localtime($datetime) : localtime();
...
sub isoUTCTimestamp {
    ...
    my ($sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst) =
        defined $datetime ? gmtime($datetime) : gmtime();
    return sprintf("%04d-%02d-%02dT%02d:%02d:%02d", $year+1900, $mon+1 ,$mday ,$hour, $min, $sec);
}
