...
sub switch_phase {
    ...
    if ((PHASE_GRAMMAR_SIMP eq $phase) and
        ...
        $out_of_ideas     = 0;
        $phase_switch     = 0;
        # Essential: We are already in PHASE_GRAMMAR_SIMP and stay with it PHASE_GRAMMAR_SIMP.
        return;
    }
...
#
    ...
    if      (PHASE_FIRST_REPLAY   eq $phase or
        ...
        if      (DURATION_ADAPTION_NONE eq $duration_adaption) {
            ...
            say("DEBUG: num_samples($num_samples), mean($mean), confidence($confidence), value_e($value_e), value($value)");
        } else {
            my $status = STATUS_INTERNAL_ERROR;
            say("INTERNAL ERROR: replay_runtime_adapt : The duration_adaption " .
        ...
        my $status = STATUS_INTERNAL_ERROR;
        say("INTERNAL ERROR: replay_runtime_adapt : The phase '$phase' is unknown. " .
            "Will exit with status " . status2text($status) . "($status)");
        Batch::emergency_exit($status);
    }
        ...
        if      ($option_to_attack eq 'reporter') {
            if ("_to_None" eq $value_to_remove) {
                ...
                if (exists $reporter_hash_copy{$value_to_remove}) {
                    ...
                    say("DEBUG: get_shrinked_rvt_options : reporter already without $value_to_remove")
                        if Auxiliary::script_debug("S6");
                }
            }
            ...
            if ("_to_None" eq $value_to_remove) {
                ...
                if (exists $validator_hash_copy{$value_to_remove}) {
                    ...
                    say("DEBUG: get_shrinked_rvt_options : validator already without $value_to_remove")
                        if Auxiliary::script_debug("S6");
                }
            }
            ...
            if ("_to_None" eq $value_to_remove) {
                ...
                if (exists $transformer_hash_copy{$value_to_remove}) {
                    ...
                    say("DEBUG: get_shrinked_rvt_options : transformer already without $value_to_remove")
                        if Auxiliary::script_debug("S6");
                }
            }
        }
