...
#                   'gendata_sql',
    ...
    # Actions of GenTest_e::Simplifier::Grammar::init
    ...
    $grammar_string = GenTest_e::Simplifier::Grammar::init($grammar_file, $threads,
                                                                  20, $grammar_flags);
    if (not defined $grammar_string) {
        say("ERROR: Simplifier::Grammar::init returned an undef grammar string.");
        my $status = STATUS_ENVIRONMENT_FAILURE;
        say("$0 will exit with exit status " . status2text($status) . "($status)");
        safe_exit($status);
    }
        ...
        if (defined $order_id) {
            ...
            my $order_is_valid = 0;
                if (PHASE_FIRST_REPLAY   eq $phase or
                    ...
                    my $cut_steps = GenTest_e::Simplifier::Grammar::estimate_cut_steps();
                    ...
                    foreach my $oid ( @oid_list ) {
                        ...
                        say("DEBUG: Simplifier::order_is_valid: oid: $oid, " .
                            ...
                            "<-") if Auxiliary::script_debug("S5");
                        my $new_rule_string = GenTest_e::Simplifier::Grammar::shrink_grammar(
    ...
    if      (PHASE_FIRST_REPLAY   eq $phase or
        ...
        while (defined $rule_name) {
            ...
            my @rule_unique_component_list =
                    GenTest_e::Simplifier::Grammar::get_unique_component_list($rule_name);
            ...
            GenTest_e::Simplifier::Grammar::set_rule_jobs_generated($rule_name);
            say("DEBUG: Rule '$rule_name' was decomposed into $success orders.")
                if Auxiliary::script_debug("S5");
            $rule_name = GenTest_e::Simplifier::Grammar::next_rule_to_process(
                             RULE_JOBS_GENERATED, RULE_WEIGHT);
        }
    } elsif (PHASE_SIMP_END eq $phase) {
    } else {
...
#
    ...
    if ($verdict eq Verdict::RQG_VERDICT_REPLAY) {
        ...
        my $gentest_runtime;
        # 2019-01-11T18:44:45 [21041] INFO: GenTest_e: Effective duration in s : 193
        ...
        if (PHASE_FIRST_REPLAY   eq $phase or
            ...
            if ($grammar_parent eq $parent_grammar) {
                ...
                foreach my $order_in_work (@orders_in_work) {
                    ...
                    my $return = GenTest_e::Simplifier::Grammar::rule_exists($rule_name);
      ...
      # The handling of replayers ends here ----------------------------------
        ...
        if (PHASE_GRAMMAR_SIMP eq $phase or
            ...
            if (PHASE_GRAMMAR_SIMP eq $phase) {
                $max_value = GenTest_e::Simplifier::Grammar::estimate_cut_steps();
...
sub switch_phase {
    ...
    if (PHASE_GRAMMAR_CLONE eq $phase ) {
        ...
        GenTest_e::Simplifier::Grammar::print_rule_hash();
        # FIXME:
        ...
        if (not defined $clone_phase) {
            ...
            $grammar_string = GenTest_e::Simplifier::Grammar::use_clones_in_grammar_top;
            if ($start_grammar_string ne $grammar_string) {
                ...
                $grammar_string = GenTest_e::Simplifier::Grammar::use_clones_in_grammar;
                if ($start_grammar_string ne $grammar_string) {
                    say("DEBUG: Cloning attempt '$clone_phase' changed the grammar.")
                        if Auxiliary::script_debug("S2");
                    unshift @simp_chain, PHASE_GRAMMAR_SIMP;
    ...
    if      (PHASE_FIRST_REPLAY eq $phase)     {
        ...
        if (1 <= $simp_success) {
            ...
            if (1 <= $grammar_simp_success) {
                $grammar_string = GenTest_e::Simplifier::Grammar::init(
...
# -> load_step will detect that and abort.
    ...
    # Actions of GenTest_e::Simplifier::Grammar::reload_grammar
    # ...  Gentest::...Simplifier ....load_grammar($grammar_file);
    ...
    $grammar_string = GenTest_e::Simplifier::Grammar::reload_grammar(
