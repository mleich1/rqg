...
# -------------------------------------------------------
# The current simplification phase (if maintained correct)
my $phase        = '';
...
my $simplify_chain;
# The current chain (== what is left over of the original chain) of simplification phases.
my @simp_chain;
#
#
...
#
# Attempts to replay with the current best replaying grammar and threads = 1,
...
# within the current best replaying grammar.
...
#
# Attempt to replay with the current best replaying grammar.
...
#    - Orders/ideas which replayed based on the current parent grammar get removed.
...
#                   'gendata_sql',
        ...
        defaults    => {
                    ...
                    'simplify_mode'     => GenTest_e::Simplifier::Grammar::SIMP_MODE_DESTRUCTIVE,
        }
    );
    ...
    if (defined $config->simplify_mode) {
        ...
        $simplify_mode =
                GenTest_e::Simplifier::Grammar::check_and_set_simplify_mode($simplify_mode);
        if (not defined $simplify_mode) {
            my $status = STATUS_ENVIRONMENT_FAILURE;
            # check_and_set_simplify_mode has already criticized the state.
            safe_exit($status);
        } else {
    ...
    if ($stop_on_replay) {
        # Replace the current @simp_chain because
...
"$iso_ts simplify_mode (used for grammar simplification)         : '$simplify_mode' (Default '" . GenTest_e::Simplifier::Grammar::SIMP_MODE_DESTRUCTIVE . "')\n"      .
...
sub generate_orders {
    ...
    if      (PHASE_FIRST_REPLAY   eq $phase or
        ...
        while (defined $rule_name) {
            $success = 0;
            if (GenTest_e::Simplifier::Grammar::SIMP_MODE_DESTRUCTIVE eq $simplify_mode) {
...
#
    ...
    if      (PHASE_FIRST_REPLAY   eq $phase or
        ...
        if      (DURATION_ADAPTION_NONE eq $duration_adaption) {
            ...
            #    (1 - z)
            # runtime properties like an current average replaying grammar.
