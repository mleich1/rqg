...
package GenTest_e::Validator::ResultsetComparatorSimplify;

require Exporter;
@ISA = qw(GenTest_e GenTest_e::Validator);
...
sub validate {
	...
	my $compare_outcome = GenTest_e::Comparator::compare($results->[0], $results->[1]);
	...
	if ($compare_outcome == STATUS_LENGTH_MISMATCH) {
		if ($query =~ m{^\s*select}io) {
			...
			say(GenTest_e::Comparator::dumpDiff($results->[0], $results->[1]));
		} else {
	...
	if (
		...
		my $simplifier_sql = GenTest_e::Simplifier::SQL->new(
			oracle => sub {
				...
				my $oracle_compare = GenTest_e::Comparator::compare($oracle_results[0], $oracle_results[1]);
		...
		if (defined $simplified_query) {
			...
			my @explains = (
				...
				$executors->[1]->execute("EXPLAIN ".$simplified_query)
	                ...
	                say("EXPLAIN diff:");
			say(GenTest_e::Comparator::dumpDiff(@explains));
			...
			say("Result set diff:");
			say(GenTest_e::Comparator::dumpDiff($simplified_results->[0], $simplified_results->[1]));

			my $simplifier_test = GenTest_e::Simplifier::Test->new(
				executors	=> $executors,
