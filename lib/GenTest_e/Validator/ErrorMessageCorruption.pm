...
package GenTest_e::Validator::ErrorMessageCorruption;
...
sub validate {
	...
	foreach my $result (@$results) {
		if (
			...
			say("Error: '".$result->errstr()."' indicates memory corruption. Note that this may be a false alarm if the test contains non-ascii character sets and non-alphanumeric characters. Terminating test with STATUS_DATABASE_CORRUPTION");
