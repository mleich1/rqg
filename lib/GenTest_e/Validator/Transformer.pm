...
package GenTest_e::Validator::Transformer;
...
sub configure {
	...
	foreach my $transformer_name (@transformer_names) {
		eval ("require GenTest_e::Transform::'".$transformer_name) or croak $@;
...
sub transform {
	...
	say(GenTest_e::Comparator::dumpDiff(@orig_explains));


	say("Simplifying...");

	my $simplifier_query = GenTest_e::Simplifier::SQL->new(
		oracle => sub {
...
#		return STATUS_WONT_HANDLE;
	...
	if (not defined $simplified_query) {
		...
		$simplifier_test = GenTest_e::Simplifier::Test->new(
			executors => [ $executor ],
	...
	#		return STATUS_WONT_HANDLE;
		...
		say("Result set diff:");
		say(GenTest_e::Comparator::dumpDiff($simplified_result, $simplified_transformed_results->[0]));
    ...
    my $translator = GenTest_e::Translator::Mysqldump2javadb->new();
    my $javadbtest = $translator->translate($test);
    if ($javadbtest) {
        $translator = GenTest_e::Translator::MysqlDML2ANSI->new();
