...
# - is inspired by the validator lib/GenTest_e/Validator/SelectStability.pm
...
#    or
#    - we run DDL and there is zero concurrent activity.
...
package GenTest_e::Validator::DmlStability;
...
sub validate {
    ...
    if ($orig_query !~ m{^\s*select}io  and
        $orig_query !~ m{^\s*update}io  and
        $orig_query !~ m{^\s*delete}io  and
        $orig_query !~ m{^\s*insert}io  and
    ...
    if (not defined $orig_result->data()) {
        if (not defined $orig_err) {
            ...
            if (
               ...
               say("DEBUG: $who_am_i Repetitions for ->$orig_query<- which harvested undef data " .
                   "and $orig_err.");
            }
        }
    } else {
