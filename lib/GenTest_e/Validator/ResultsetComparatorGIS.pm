...
package GenTest_e::Validator::ResultsetComparatorGIS;
...
#		say("Problematic query: ".$results->[0]->query()."\n");
	...
	if (defined $geometries[0] && defined $geometries[1]) {
		...
		if ($area_results->[0]->status() == STATUS_OK && $area_results->[1]->status() == STATUS_OK) {
			my $compare_outcome_areas = GenTest_e::Comparator::compare($area_results->[0], $area_results->[1]);
	...
	if ($compare_outcome == STATUS_LENGTH_MISMATCH) {
		...
		say(GenTest_e::Comparator::dumpDiff($results->[0], $results->[1]));
		say(GenTest_e::Comparator::dumpDiff($area_results->[0], $area_results->[1])) if defined $area_results->[0] && defined $area_results->[1];
	}
