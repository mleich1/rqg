...
package GenTest_e::Generator::FromGrammar;

require Exporter;
@ISA = qw(GenTest_e::Generator GenTest_e);

use strict;
use GenTest_e::Constants;
use GenTest_e::Random;
use GenTest_e::Generator;
use GenTest_e::Grammar;
use GenTest_e::Grammar::Rule;
use GenTest_e::Stack::Stack;
use GenTest_e;
use Cwd;
...
sub new {
    ...
    #   say("DEBUG: Loading grammar file '" . $generator->grammarFile() . "' ...");
        $generator->[GENERATOR_GRAMMAR] = GenTest_e::Grammar->new(
    ...
    if (not defined $generator->prng()) {
        $generator->[GENERATOR_PRNG] = GenTest_e::Random->new(
...
sub globalFrame {
   my ($self) = @_;
   $self->[GENERATOR_GLOBAL_FRAME] = GenTest_e::Stack::StackFrame->new()
...
sub next {
    ...
    our $who_am_i = "GenTest_e::Generator::FromGrammar::next:";
    ...
    my $stack = GenTest_e::Stack::Stack->new();
    ...
    sub expand {
        ...
        #     Deep recursion on subroutine "GenTest_e::Generator::FromGrammar::expand"
        #     at /work/RQG_mleich3/lib/GenTest_e/Generator/FromGrammar.pm line ....
        # --------------------------------------------------------------------------
            ...
            if (exists $grammar_rules->{$item}) {
                ...
                if ($invariant) {
                  ...
                  # }
                    ...
                    # say("DEBUG: item ->$item<-");
                    @expansion = expand($rule_counters,$rule_invariants,@{$grammar_rules->{$item}->[GenTest_e::Grammar::Rule::RULE_COMPONENTS]->[
                        $prng->uint16(0, $#{$grammar_rules->{$item}->[GenTest_e::Grammar::Rule::RULE_COMPONENTS]})
                    ]});
                }
