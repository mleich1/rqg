...
package GenTest_e::Reporter::ReplicationSemiSync;
...
sub monitor {
	...
	say("GenTest_e::Reporter::ReplicationSemiSync: Test cycle starting.");
...
# 	) {
# 		say("GenTest_e::Reporter::ReplicationSemiSync: Semisync replication is not enabled: rpl_semi_sync_master_status = $rpl_semi_sync_master_status_first.");
# 		return STATUS_REPLICATION_FAILURE;
# 	}

#	$master_dbh->do("SET GLOBAL rpl_semi_sync_master_timeout = ".($rpl_semi_sync_master_timeout * 1000));
#	say("GenTest_e::Reporter::ReplicationSemiSync: Acquiring the global read lock.");
#	$master_dbh->do("FLUSH NO_WRITE_TO_BINLOG TABLES WITH READ LOCK");
#	say("GenTest_e::Reporter::ReplicationSemiSync: stopping slave IO thread.");
#	$slave_dbh->do("STOP SLAVE IO_THREAD");
#	say("GenTest_e::Reporter::ReplicationSemiSync: stopped slave IO thread.");

#	return STATUS_REPLICATION_FAILURE if isSlaveBehind($master_dbh, $slave_dbh);

#	$master_dbh->do("FLUSH NO_WRITE_TO_BINLOG STATUS");
#	say("GenTest_e::Reporter::ReplicationSemiSync: Flushed status.");
#	$master_dbh->do("UNLOCK TABLES");
#	say("GenTest_e::Reporter::ReplicationSemiSync: Released the global read lock.");
...
#	my ($unusedB, $rpl_semi_sync_master_no_tx_atflush) = $master_dbh->selectrow_array("SHOW STATUS LIKE 'Rpl_semi_sync_master_no_tx'");
	...
	say("GenTest_e::Reporter::ReplicationSemiSync: Sleeping for $sleep_interval seconds.");
	...
	if ($sleep_interval > $rpl_semi_sync_master_timeout) {
                if ($rpl_semi_sync_master_status_after eq 'ON') {
                        say("GenTest_e::Reporter::ReplicationSemiSync: rpl_semi_sync_master_status = ON even after stopping for longer than the timeout.");
                        return STATUS_REPLICATION_FAILURE;
                } elsif ($rpl_semi_sync_master_no_tx == 0) {
			say("GenTest_e::Reporter::ReplicationSemiSync: Transactions were not committed asynchronously while slave was stopped for longer than the timeout.");
			say("GenTest_e::Reporter::ReplicationSemiSync: rpl_semi_sync_master_no_tx = $rpl_semi_sync_master_no_tx;");
		} elsif ($rpl_semi_sync_master_yes_tx > 0) {
			say("GenTest_e::Reporter::ReplicationSemiSync: Transactions were committed semisynchronously while slave was stopped longer than the timeout.");
			say("GenTest_e::Reporter::ReplicationSemiSync: rpl_semi_sync_master_yes_tx = $rpl_semi_sync_master_yes_tx;");
			return STATUS_REPLICATION_FAILURE;
		}
	} else {
...
#
		if ($rpl_semi_sync_master_status_after eq 'OFF') {
			say("GenTest_e::Reporter::ReplicationSemiSync: rpl_semi_sync_master_status = OFF even after stopping for less than the timeout.");
			return STATUS_REPLICATION_FAILURE;
		} elsif ($rpl_semi_sync_master_no_tx > 0) {
			say("GenTest_e::Reporter::ReplicationSemiSync: Transactions were committed asynchronously while slave was stopped for less than the timeout.");
			say("GenTest_e::Reporter::ReplicationSemiSync: rpl_semi_sync_master_no_tx = $rpl_semi_sync_master_no_tx;");
			return STATUS_REPLICATION_FAILURE;
		} elsif ($rpl_semi_sync_master_yes_tx > 0) {
			say("GenTest_e::Reporter::ReplicationSemiSync: Transactions were committed semisynchronously while slave was stopped for less than the timeout.");
			say("GenTest_e::Reporter::ReplicationSemiSync: rpl_semi_sync_master_yes_tx = $rpl_semi_sync_master_yes_tx;");
			return STATUS_REPLICATION_FAILURE;
		} else {
#			return STATUS_REPLICATION_FAILURE if isSlaveBehind($master_dbh, $slave_dbh);
	...
	say("GenTest_e::Reporter::ReplicationSemiSync: Starting slave IO thread.");
...
# 		say("GenTest_e::Reporter::ReplicationSemiSync: Master has failed to return to semisync replication even after the slave has reconnected.");
# 		return STATUS_REPLICATION_FAILURE;
# 	}

# 	say("GenTest_e::Reporter::ReplicationSemiSync: test cycle ending with Rpl_semi_sync_master_status = $rpl_semi_sync_master_status_last.");
...
sub isSlaveBehind {
	...
	say("GenTest_e::Reporter::ReplicationSemiSync: slave: master_log_file = $master_log_file; read_master_log_pos = $read_master_log_pos; master_log_id = $master_log_id.");
	if ( 
		...
		say("GenTest_e::Reporter::ReplicationSemiSync: Slave has lagged behind while Rpl_semi_sync_master_status = $rpl_semi_sync_master_status.");
		return STATUS_REPLICATION_FAILURE;
	}
}
...
sub waitForSlave {
	...
	say("GenTest_e::Reporter::ReplicationSemiSync: Flushing tables with read lock on master...");
	$master_dbh->do("FLUSH NO_WRITE_TO_BINLOG TABLES WITH READ LOCK");
	say("GenTest_e::Reporter::ReplicationSemiSync: ... flushed.");

	my ($file, $pos) = $master_dbh->selectrow_array("SHOW MASTER STATUS");

	if (($file eq '') || ($pos eq '')) {
		 say("GenTest_e::Reporter::ReplicationSemiSync: SHOW MASTER STATUS failed.");
		 return STATUS_REPLICATION_FAILURE;
	}

	say("GenTest_e::Reporter::ReplicationSemiSync: Waiting for slave...");
	#say("SHOW MASTER STATUS: " . $file . ", " . $pos);
	my $wait_status = $slave_dbh->selectrow_array("SELECT MASTER_POS_WAIT(?, ?)", undef, $file, $pos);
	say("GenTest_e::Reporter::ReplicationSemiSync: ... slave caught up with master.");
	...
	if (not $rpl_semi_sync_master_status eq 'ON') {
	    say("GenTest_e::Reporter::ReplicationSemiSync: Master has failed to return to semisync replication even after the slave has caught up.");
	    return STATUS_REPLICATION_FAILURE;
	}
	...
	if (not defined $wait_status) {
		say("GenTest_e::Reporter::ReplicationSemiSync: MASTER_POS_WAIT() has failed. Slave SQL thread has likely stopped.");
		return STATUS_REPLICATION_FAILURE;
	}
	return 0;
}
