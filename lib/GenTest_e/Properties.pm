...
# FIXME: (mleich)
# Make a clone GenTest_e::Properties1 which does not croak, die, exit
...
## my $config=GenTest_e::Properties->new(options=>$options......);

##
package GenTest_e::Properties;
...
sub AUTOLOAD {
    ...
    if (defined $self->[PROPS_LEGAL_HASH]) {
        if ( not $self->[PROPS_LEGAL_HASH]->{$name} ) {
           ...
           Carp::confess("ERROR: Illegal property '$name' caught by GenTest_e::Properties::AUTOLOAD.\n" .
               "Will exit.");
           # return undef;
        }
    } else {
