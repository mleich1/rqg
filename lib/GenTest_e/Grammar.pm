...
sub parseFromString {
    ...
    foreach my $rule_name (sort keys %rules) {
        ...
        my $rule = GenTest_e::Grammar::Rule->new(
            name       => $rule_name,
...
# - GenTest_e/Grammar.pm does not "know" the number of threads to be used.
#
...
sub cloneRule {
    ...
    my $new_rule = GenTest_e::Grammar::Rule->new(
        name       => $new_rule_name,
...
sub patch {
    ...
    my $new_grammar = GenTest_e::Grammar->new(grammar_rules => $rules);
    return $new_grammar;
}
...
sub topGrammar {
    ...
    return GenTest_e::Grammar->new(grammar_rules => $rules);
}
...
sub mask {
    ...
    my $i = 0;
    my $prng = GenTest_e::Random->new(seed => $mask);
    ...
    foreach my $rulename (sort keys %$rules) {
        ...
        if ($#newComponents < 0) {
            ...
            $newRule= GenTest_e::Grammar::Rule->new(name       => $rulename,
    ...
    return GenTest_e::Grammar->new(grammar_rules => \%newRuleset);
}
