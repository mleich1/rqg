...
sub run {
    ...
    say("INFO: Starting GenTest_e::App::Gendata");

    my $spec_file = $self->spec_file();

    my $prng = GenTest_e::Random->new(
        seed => $self->seed(),
        varchar_length => $self->varchar_length()
    );
    Carp::cluck("WARN: FIXME varchar_length is undef") if not defined $self->varchar_length();
...
#
    ...
    if ($spec_file ne '') {
        ...
        if (not open(CONF , $spec_file)) {
            say("FATAL ERROR: unable to open specification file '$spec_file': $!");
            return STATUS_ENVIRONMENT_FAILURE;
        }
        read(CONF, my $spec_text, -s $spec_file);
        eval ($spec_text);
        if ('' ne $@) {
            say("FATAL ERROR: Unable to load '$spec_file': $@");
            return STATUS_ENVIRONMENT_FAILURE;
        }
    }

    if ($executor->type == DB_MYSQL) {
        my $result = $executor->execute("SET SQL_MODE= CONCAT(\@\@sql_mode,',NO_ENGINE_SUBSTITUTION')");
        my $status = $result->status;
        return $status if $status != STATUS_OK;
    }
    if ((defined $self->engine() and $self->engine() ne '') and
        ($executor->type == DB_MYSQL or $executor->type == DB_DRIZZLE)) {
        my $result = $executor->execute("SET DEFAULT_STORAGE_ENGINE= '" . $self->engine() . "'");
        my $status = $result->status;
        return $status if $status != STATUS_OK;
    }
...
# If no fields were defined, continue with just the primary key.
    ...
    @fields = () if ($#fields == 0) && (not defined $fields[0]->[FIELD_TYPE] or $fields[0]->[FIELD_TYPE] eq '');
    ...
    foreach my $field_id (0..$#fields) {
             ...
             && (defined $field_copy[FIELD_DEFAULT] and $field_copy[FIELD_DEFAULT]=~ m{default null}sio) )
        {
	        ...
	        # We dont want to keep this, because it causes duplicate columns to be created.
    ...
    foreach my $schema (@schema_perms) {
        my $result = $executor->execute("CREATE SCHEMA /*!IF NOT EXISTS*/ $schema");
        my $status = $result->status;
        return $status if $status != STATUS_OK;
        $executor->sqltrace($self->sqltrace);
        if (not defined $executor->currentSchema($schema)) {
            say("ERROR: The call of currentSchema failed. Will return STATUS_CRITICAL_FAILURE.");
    ...
    foreach my $table_id (0..$#tables) {
        ...
        my $result = $executor->execute("DROP TABLE /*! IF EXISTS*/ $table->[TABLE_NAME]");
        my $status = $result->status;
        return $status if $status != STATUS_OK;
        ...
        $result = $executor->execute("CREATE TABLE `$table->[TABLE_NAME]` (\n".join(",\n/*Indices*/\n", grep { defined $_ } (@field_sqls, $index_sqls) ).") ".$table->[TABLE_SQL]);
        # This can harvest a semantic error like
        # CREATE TABLE ... PARTITION BY key (pk)    and pk does not exist
        # 1488 Field in list of fields for partition function not found in table. Further errors of this kind will be suppressed.
        # because of combinatorics.
        # Example:
        # pk => [undef,'int','int auto_increment','varchar(10)'],
        ...
        if ($executor->type == DB_MYSQL ) {
            ...
            # Experiment:
            # Bundling up to 10000 inserted rows (50 rows per INSERT statement, at end less) into
            # one COMMIT makes most probably serious stress when generating bigger tables.
            # So why not use bigger bundles even for the smaller tables.
        ...
        # 1. Use chunks consisting of 50 rows (the last one is smaller) per single INSERT statement.
        #    --> Some stress caused by statement size, less frequent client to server communication.
        # 2. Use IGNORE
        # 2. Commit after having inserted the last chunk.
        # 3. Commit if having row number % 10000 == 0.
        # 4. If number of rows < 100 go must probably with autocommit.
        #        --> Number of rows to autocommit at once <= 50.
        #    If number of rows > 100, disable autocommit and start a transaction.
        #        --> Number of rows to commit at once <= 10000.
        # Commit after having inserted the last chunk.
        my @row_buffer;
        foreach my $row_id (1..$table->[TABLE_ROW]) {
            ...
            if ((($row_id % 50) == 0) || ($row_id == $table->[TABLE_ROW])) {
                # We go with 50 rows per single INSERT statement or some smaller amount at end.
                ...
                # causing too frequent wrong syntax.
                my $insert_result = $executor->execute(
                "INSERT /*! IGNORE */ INTO $table->[TABLE_NAME] VALUES \n" .
                join(",\n", @row_buffer));
            ...
            if ($executor->type == DB_MYSQL ) {
                ...
                $result = $executor->execute("ALTER TABLE `$table->[TABLE_NAME]` ENABLE KEYS");
                $status = $result->status;
                return $status if $status != STATUS_OK;
            }
        }
    ...
    if ((defined $table_perms[TABLE_MERGES]) && ($#myisam_tables > -1)) {
        foreach my $merge_id (0..$#{$table_perms[TABLE_MERGES]}) {
            my $merge_name = 'merge_' . $merge_id;
            my $result = $executor->execute("CREATE TABLE `$merge_name` LIKE `" .
                                            $myisam_tables[0] . "`");
            my $status = $result->status;
            return $status if $status != STATUS_OK;

            $result = $executor->execute("ALTER TABLE `$merge_name` ENGINE=MERGE UNION(" .
                                         join(',', @myisam_tables) . ") " .
                                         uc($table_perms[TABLE_MERGES]->[$merge_id]));
            $status = $result->status;
            return $status if $status != STATUS_OK;
        }
    }
    ...
    say("INFO: Finishing GenTest_e::App::Gendata");
