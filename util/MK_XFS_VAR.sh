...
# Thoughts about a RQG Combinator config file having 50% vardir_fast and 50% vardir_slow
# --------------------------------------------------------------------------------------
# 1. Observation:
...
#    (**) Analysis is frequent extreme difficult.
# 2. Raising the size of rqg_ext4 over 50% makes no sense because certain stuff
#    (Example: rr traces, file backups, ...) of tests using slowdir get already
#    stored in /dev/shm. I guess that some raise would led to reducing the
#    overall throughput.
...
#   maximum throughput == only low fraction of tests stopped because of trouble
#   with free space in fast_dir (/dev/shm/rqg) and slow_dir (maybe /dev/shm/rqg_ext4)
